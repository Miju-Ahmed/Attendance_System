...
PROCESS_EVERY_N_FRAMES = 2  # Process every Nth frame (1=all frames, 2=every other frame, 3=every 3rd)
...
class FaceBasedAttendanceSystem:
    ...
    def process_frame(self, frame: np.ndarray, skip_detection: bool = False) -> np.ndarray:
        ...
        # Step 1: Detect faces and embeddings (skip on some frames for performance)
        if skip_detection:
            face_detections = []
        else:
        ...
        if self.line_detector is not None:
            for track_id, track_info in list(self.track_identities.items()):
                ...
                # Require recent detection to avoid false crossings from predictions
                if self.frame_idx - track_info['last_seen'] > 3:
                    continue
                ...
                if stable_id not in self.track_line_state:
                    ...
                    self.track_line_state[stable_id] = {
                        'last_side': curr_side,  # Set initial side
                        'cooldown': 0,
                        'state': initial_state
                    }
                    logging.info(f"🆕 Initialized {track_info['name']} (ID:{stable_id}) state: {initial_state}, side: {curr_side}, pos: {curr_pos}")
                ...
                if line_state['last_side'] is not None and line_state['cooldown'] == 0:
                    ...
                    if event == "ENTRY" and line_state['state'] == "OUTSIDE":
                        ...
                        logging.debug(f"⚠️  {track_info['name']} already INSIDE - ignoring duplicate entry")
                        ...
                        logging.debug(f"⚠️  {track_info['name']} already OUTSIDE - ignoring exit without entry")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                skip_detection = (self.frame_idx % self.process_every_n) != 0
                
                processed_frame = self.process_frame(frame, skip_detection=skip_detection)
