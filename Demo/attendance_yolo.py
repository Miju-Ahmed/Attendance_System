"""
Real-Time Attendance System (YOLO + SCRFD + ArcFace + SQLite)
=============================================================
Requirements:
- YOLO: person detection only (tracking by body boxes).
...
- No motion prediction: tracks update only when YOLO detects a body.
"""
...
class AttendanceDatabase:
    ...
    def record_event(self, name: str, stable_id: int, event_type: str) -> None:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.cursor.execute(
            """
            INSERT INTO attendance (person_name, stable_id, event_type, timestamp)
            VALUES (?, ?, ?, ?)
            """,
            (name, stable_id, event_type, timestamp),
        )
        self.conn.commit()
        logging.info("%s: %s (ID:%s) at %s", event_type, name, stable_id, timestamp)
...
class IoUTracker:
    ...
    def update(self, detections: List[np.ndarray], frame_idx: int) -> List[Track]:
        ...
        # Return tracks updated this frame (no prediction)
...
class AttendanceYOLO:
    def __init__(self, args: argparse.Namespace):
        ...
        self.face_recognizer = ArcFace(model_path=args.face_rec_model, providers=providers)

        if YOLO is None:
            raise RuntimeError("ultralytics not installed; YOLO person detection is required.")
    ...
    def detect_faces(self, frame: np.ndarray) -> List[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        ...
        try:
            ...
            logging.debug("Face detection error: %s", exc)
        return faces

    def detect_persons(self, frame: np.ndarray) -> List[np.ndarray]:
        results = self.yolo_model(
            frame, conf=self.args.yolo_conf, classes=[PERSON_CLASS_ID], verbose=False
        )
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Track bodies with IoU (no prediction)
            ...
            for track in updated_tracks:
                ...
                if state["cooldown"] == 0:
                    ...
                    if entry_trigger and state["state"] in ("UNKNOWN", "OUTSIDE"):
                        self.db.record_event(track.name, track.stable_id, "ENTRY")
                        self.entry_count += 1
                        state["state"] = "INSIDE"
                        state["cooldown"] = COOLDOWN_FRAMES
                    elif exit_trigger and state["state"] in ("UNKNOWN", "INSIDE"):
                        self.db.record_event(track.name, track.stable_id, "EXIT")
                        self.exit_count += 1
                        state["state"] = "OUTSIDE"
                        state["cooldown"] = COOLDOWN_FRAMES
        ...
        for track in updated_tracks:
            ...
            if track.authorized and track.name and track.stable_id is not None:
                color = (0, 255, 0)
                label = f"ID:{track.stable_id} {track.name}"
            else:
                color = (0, 165, 255)
        ...
        try:
            while True:
                ...
                if not self.args.skip_display:
                    cv2.imshow("Attendance YOLO", processed)
                    key = cv2.waitKey(1) & 0xFF
                    if key == ord("q"):
                        break
                self.frame_idx += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Attendance System (YOLO + SCRFD + ArcFace + SQLite)")
